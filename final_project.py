...
def main():
    ...
    if os.path.isfile('weights.npy') & os.path.isfile('biases.npy'):
        inner_layers[0].load(np.load('weights.npy'), np.load('biases.npy'))
    ...
    np.save ('weights.npy', inner_layers[0].weights) #, delimiter = ",")
    np.save ('biases.npy', inner_layers[0].bias) #, delimiter = ",")
